...
public partial class Default2 : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            //SqlCommand cmd = new SqlCommand("select * from booking where vendor_email='" + Session["email"] + "'", con);
            //SqlDataAdapter das = new SqlDataAdapter(cmd);
            //DataSet dsa = new DataSet();
            //con.Open();
            //cmd.ExecuteScalar();
            //das.Fill(dsa);
            //Repeater1.DataSource = dsa.Tables[0];
            //Repeater1.DataBind();
            //con.Close();
        }
        catch
    ...
    {
        SqlConnection con = new SqlConnection(strConnString);
        SqlCommand cd = new SqlCommand("update booking set status='" + DropDownList1.SelectedValue + "' where vendor_email='" + Label1.Text + "' AND email='" + Label2.Text + "'", con);
        con.Open();
        cd.ExecuteNonQuery();
